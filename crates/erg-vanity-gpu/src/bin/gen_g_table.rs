//! Generate G_TABLE (8-bit windowed k·G) and the 8-bit comb table.
//!
//! cargo run -p erg-vanity-gpu --bin gen_g_table

use erg_vanity_crypto::secp256k1::field::FieldElement;
use erg_vanity_crypto::secp256k1::point::Point;
use erg_vanity_crypto::secp256k1::scalar::Scalar;
use std::fs;
use std::io::{self, Write};
use std::path::PathBuf;

const EXPECTED_GX: [u32; 8] = [
    0x16F81798, 0x59F2815B, 0x2DCE28D9, 0x029BFCDB, 0xCE870B07, 0x55A06295, 0xF9DCBBAC, 0x79BE667E,
];
const EXPECTED_GY: [u32; 8] = [
    0xFB10D4B8, 0x9C47D08F, 0xA6855419, 0xFD17B448, 0x0E1108A8, 0x5DA4FBFC, 0x26A3C465, 0x483ADA77,
];

/// Comb window width in bits. Every other comb constant derives from it.
///
/// Wider windows mean fewer mixed adds per k·G, but the table grows as 2^bits
/// and is read with one scattered lookup per window:
///
/// | bits | windows | entries | table | max adds |
/// |---|---|---|---|---|
/// | 8 | 32 | 256 | 512 KB | 31 |
/// | 9 | 29 | 512 | 950 KB | 28 |
/// | 10 | 26 | 1024 | 1.7 MB | 25 |
/// | 11 | 24 | 2048 | 3.1 MB | 23 |
///
/// Changing this requires regenerating `kernels/comb_table.bin` and updating
/// the matching constants in `comb.rs` and `secp256k1_point.cl`.
const COMB_BITS: usize = 11;
const COMB_WINDOWS: usize = 256_usize.div_ceil(COMB_BITS);
const COMB_ENTRIES: usize = 1 << COMB_BITS;
/// Bits taken by window 0. Windows 1.. take `COMB_BITS` each.
const COMB_TOP_BITS: usize = 256 - (COMB_WINDOWS - 1) * COMB_BITS;
const COMB_XY_LIMBS: usize = 16;
const COMB_BYTES: usize = COMB_WINDOWS * COMB_ENTRIES * COMB_XY_LIMBS * 4;

fn bytes_to_limbs(bytes: &[u8; 32]) -> [u32; 8] {
    let mut limbs = [0u32; 8];
    for (i, limb) in limbs.iter_mut().enumerate() {
        let off = (7 - i) * 4;
        *limb = ((bytes[off] as u32) << 24)
            | ((bytes[off + 1] as u32) << 16)
            | ((bytes[off + 2] as u32) << 8)
            | (bytes[off + 3] as u32);
    }
    limbs
}

fn limbs_to_bytes(limbs: &[u32; 8]) -> [u8; 32] {
    let mut bytes = [0u8; 32];
    for (i, limb) in limbs.iter().enumerate() {
        let off = (7 - i) * 4;
        bytes[off..off + 4].copy_from_slice(&limb.to_be_bytes());
    }
    bytes
}

fn point_limbs(i: u16) -> ([u32; 8], [u32; 8], [u32; 8]) {
    if i == 0 {
        let one = [1u32, 0, 0, 0, 0, 0, 0, 0];
        return (one, one, [0u32; 8]);
    }
    let mut bytes = [0u8; 32];
    bytes[30] = (i >> 8) as u8;
    bytes[31] = i as u8;
    let scalar = Scalar::from_bytes(&bytes).expect("window scalar in range");
    let point = Point::mul_generator(&scalar);
    let (x, y) = point.to_affine().expect("i*G is not infinity");
    let z_one = [1u32, 0, 0, 0, 0, 0, 0, 0];
    (
        bytes_to_limbs(&x.to_bytes()),
        bytes_to_limbs(&y.to_bytes()),
        z_one,
    )
}

fn write_g_table<W: Write>(mut out: W) -> io::Result<()> {
    writeln!(
        out,
        "// Precomputed table: G_TABLE[i] = i * G for i = 0..255"
    )?;
    writeln!(
        out,
        "// Each point = 24 uints (X[8], Y[8], Z[8]), limb 0 = LSB."
    )?;
    writeln!(
        out,
        "// Generated by: cargo run -p erg-vanity-gpu --bin gen_g_table"
    )?;
    writeln!(out, "__constant uint G_TABLE[256][24] = {{")?;

    for i in 0..256u16 {
        let (x, y, z) = point_limbs(i);
        if i == 1 {
            assert_eq!(x, EXPECTED_GX, "1*G X mismatch");
            assert_eq!(y, EXPECTED_GY, "1*G Y mismatch");
            eprintln!("1*G matches kernel GX/GY");
        }
        write!(out, "    {{")?;
        for (j, limb) in x.iter().chain(y.iter()).chain(z.iter()).enumerate() {
            if j > 0 {
                write!(out, ",")?;
            }
            write!(out, " {limb:#010X}u")?;
        }
        if i < 255 {
            writeln!(out, " }},")?;
        } else {
            writeln!(out, " }}")?;
        }
    }
    writeln!(out, "}};")?;
    Ok(())
}

fn store_xy(buf: &mut [u8], window: usize, b: usize, x: &[u32; 8], y: &[u32; 8]) {
    let off = (window * COMB_ENTRIES + b) * COMB_XY_LIMBS * 4;
    for (i, limb) in x.iter().chain(y.iter()).enumerate() {
        let o = off + i * 4;
        buf[o..o + 4].copy_from_slice(&limb.to_le_bytes());
    }
}

fn load_affine(buf: &[u8], window: usize, b: usize) -> Point {
    if b == 0 {
        return Point::INFINITY;
    }
    let off = (window * COMB_ENTRIES + b) * COMB_XY_LIMBS * 4;
    let mut x_limbs = [0u32; 8];
    let mut y_limbs = [0u32; 8];
    for i in 0..8 {
        x_limbs[i] = u32::from_le_bytes(buf[off + i * 4..off + i * 4 + 4].try_into().unwrap());
        y_limbs[i] =
            u32::from_le_bytes(buf[off + 32 + i * 4..off + 36 + i * 4].try_into().unwrap());
    }
    let x = FieldElement::from_bytes(&limbs_to_bytes(&x_limbs)).expect("comb X in field");
    let y = FieldElement::from_bytes(&limbs_to_bytes(&y_limbs)).expect("comb Y in field");
    Point::from_affine(x, y)
}

/// Rebuild k·G from the table exactly the way `pt_mul_generator_comb` does, so
/// the generator is checked against the consumer's indexing and not just its
/// own.
fn reconstruct(buf: &[u8], k_bytes: &[u8; 32]) -> Point {
    // Window 0 takes the top COMB_TOP_BITS bits of the MSB-first scalar.
    let first = (k_bytes[0] >> (8 - COMB_TOP_BITS)) as usize;
    let mut acc = load_affine(buf, 0, first);
    let mut bit = COMB_TOP_BITS;
    for window in 1..COMB_WINDOWS {
        let mut digit = 0usize;
        for _ in 0..COMB_BITS {
            digit = (digit << 1) | ((k_bytes[bit / 8] >> (7 - bit % 8)) & 1) as usize;
            bit += 1;
        }
        acc = acc.add(&load_affine(buf, window, digit));
    }
    acc
}

fn build_comb_table() -> Vec<u8> {
    let mut buf = vec![0u8; COMB_BYTES];
    let mut base = Point::generator();

    for window in (0..COMB_WINDOWS).rev() {
        let mut acc = Point::INFINITY;
        for b in 1..COMB_ENTRIES {
            acc = acc.add(&base);
            let (x, y) = acc.to_affine().expect("b*base is not infinity");
            store_xy(
                &mut buf,
                window,
                b,
                &bytes_to_limbs(&x.to_bytes()),
                &bytes_to_limbs(&y.to_bytes()),
            );
        }
        if window > 0 {
            for _ in 0..COMB_BITS {
                base = base.double();
            }
        }
    }

    // The last window holds the low bits, so its entry 1 is G itself.
    let last = COMB_WINDOWS - 1;
    let g = load_affine(&buf, last, 1);
    let (gx, gy) = g.to_affine().expect("T[last][1] is G");
    assert_eq!(
        bytes_to_limbs(&gx.to_bytes()),
        EXPECTED_GX,
        "comb T[last][1] X"
    );
    assert_eq!(
        bytes_to_limbs(&gy.to_bytes()),
        EXPECTED_GY,
        "comb T[last][1] Y"
    );
    eprintln!("comb T[{last}][1] matches G");

    let two = load_affine(&buf, last, 2);
    assert_eq!(two, Point::generator().double(), "comb T[last][2] is 2G");

    for k_bytes in [
        [0u8; 32],
        {
            let mut k = [0u8; 32];
            k[31] = 1;
            k
        },
        {
            let mut k = [0u8; 32];
            k[31] = 3;
            k
        },
        {
            let mut k = [0u8; 32];
            k[0] = 0x01;
            k[15] = 0xAB;
            k[31] = 0x7F;
            k
        },
    ] {
        let scalar = Scalar::from_bytes(&k_bytes).expect("test scalar in range");
        assert_eq!(
            reconstruct(&buf, &k_bytes),
            Point::mul_generator(&scalar),
            "comb reconstruct mismatch"
        );
    }
    // Random scalars: full-width coverage of every window/digit path.
    for _ in 0..64 {
        let k_bytes: [u8; 32] = rand::random();
        let scalar = match Scalar::from_bytes(&k_bytes) {
            Some(s) => s,
            None => continue, // k >= n, skip
        };
        assert_eq!(
            reconstruct(&buf, &k_bytes),
            Point::mul_generator(&scalar),
            "comb reconstruct mismatch (random)"
        );
    }
    eprintln!("comb reconstruct checks passed");

    buf
}

fn main() {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("kernels");

    let g_dest = dir.join("g_table.cl");
    let mut g_buf = Vec::new();
    write_g_table(&mut g_buf).unwrap();
    fs::write(&g_dest, g_buf).unwrap();
    eprintln!("wrote {}", g_dest.display());

    let comb_dest = dir.join("comb_table.bin");
    let comb = build_comb_table();
    fs::write(&comb_dest, &comb).unwrap();
    eprintln!("wrote {} ({} bytes)", comb_dest.display(), comb.len());
}
