...
fn main() {
    ...
    if args.list_devices {
        ...
        match erg_vanity_gpu::cuda::CudaDevice::enumerate() {
            Ok(list) if !list.is_empty() => {
                println!();
                println!("CUDA devices (ERG_BACKEND=cuda):");
                for d in &list {
                    println!(
                        "  [{}] {} - {} ({} CUs)",
                        d.global_idx, d.platform_name, d.device_name, d.compute_units
                    );
                }
            }
            Ok(_) => {
                if erg_vanity_gpu::cuda::pipeline::CUDA_BUILT {
