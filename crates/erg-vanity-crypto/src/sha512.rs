...
//! Used by HMAC-SHA512 for PBKDF2.
...
/// Pad message according to FIPS 180-4 Section 5.1.2.
fn pad(data: &[u8]) -> Vec<u8> {
    ...
    // Append zeros until length ≡ 896 (mod 1024), i.e., 112 bytes (mod 128)
...
/// Compress one 1024-bit block into the state.
...
mod tests {
    ...
    fn test_against_sha2_crate() {
        ...
        let test_cases = [
            ...
            &[0u8; 129], // just over one block
            &[0u8; 256], // exactly two blocks
        ];

        for data in test_cases {
            let our_result = digest(data);
            let ref_result: [u8; 64] = Sha512::digest(data).into();
            assert_eq!(
                our_result,
                ref_result,
